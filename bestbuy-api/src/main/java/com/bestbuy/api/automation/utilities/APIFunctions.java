...
public class APIFunctions {
    ...
    public static Response getRequest(int limit, int skip, String url) {
        response = SerenityRest.given().param("$limit", limit).param("$skip", skip).when()
    ...
    public static Response getRequest(Long id, String url) {
        response = SerenityRest.given().when()
                .get(GenericFunctions.getConfigProperties().getProperty("baseURI").concat(url + "/" + id));
        return response;
    }

    public static <T> Response postRequest(T Object, String url) {
        response = SerenityRest.given().header("Content-Type", "application/json").when().body(Object)
    ...
    public static <T> Response patchRequest(Long id, T Object, String url) {
        response = SerenityRest.given().header("Content-Type", "application/json").when().body(Object)
                .patch(GenericFunctions.getConfigProperties().getProperty("baseURI").concat(url + "/" + id));
        return response;
    }

    public static <T> Response deleteRequest(Long id, String url) {
        response = SerenityRest.given().header("Content-Type", "application/json").when()
                .delete(GenericFunctions.getConfigProperties().getProperty("baseURI").concat(url + "/" + id));
        return response;
    }
